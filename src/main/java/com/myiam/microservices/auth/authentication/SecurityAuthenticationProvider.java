...
class SecurityAuthenticationProvider implements AuthenticationProvider {
     ...
     * ※現行はユーザーパスワードのみ支援する
     *
     * @param authentication 認証クラスの件所対象
     ...
     */
    private UserDto.@NonNull AuthUserView getAuthUser(Authentication authentication) throws AuthenticationException {
        // ユーザー識別子をキーに該当するユーザー詳細情報を照会
        UserDto.AuthUserView user = userQueryService.findUserByUsername(authentication.getName());
     ...
     * @return ファクター付与権限
     */
    private @NonNull FactorGrantedAuthority verifyCredentials(@NonNull Authentication authentication, UserDto.AuthUserView user) {
     ...
     * @return 認証トークン
     */
    private static @NonNull UsernamePasswordAuthenticationToken getAuthenticatedToken(UserDto.AuthUserView user, FactorGrantedAuthority factor) {
        // 権限リストにファクター付与権限を追加
        Set<GrantedAuthority> authorities = new HashSet<>(user.authorities());
        authorities.add(factor);

        // ユーザ情報作成
        var userView = UserDto.UserView.builder()
                .id(user.id())
